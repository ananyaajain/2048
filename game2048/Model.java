...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < _board.size(); col += 1) {
            boolean[] updated = new boolean[_board.size()];
            for (int row = _board.size() - 1; row >= 0; row -= 1) {
                if (row == _board.size() - 1 || _board.tile(col, row) == null) {
                    continue;
                }
                int loc = row;
                while (loc < _board.size()) {
                    if ((loc+1)>=_board.size()){
                        break;
                    }
                    if (_board.tile(col, loc + 1) == null) {
                        loc++;
                        needToChange = true;
                    } else if (_board.tile(col, loc + 1) != null && _board.tile(col, row).value() == _board.tile(col, loc + 1).value()) {
                        if (updated[loc + 1]) {
                            break;
                        } else {
                            loc++;
                            updated[loc] = true;
                            _score += _board.tile(col, loc).value() * 2;
                            needToChange = true;
                            break;
                        }
                    }
                ...
                _board.move(col, loc, tile(col, row));
            }
        }
